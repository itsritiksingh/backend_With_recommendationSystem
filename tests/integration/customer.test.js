...
describe('Customar',() => {
  ...
  const exec = async () => {
    return await request(server)
          ...
          .send({name,email,address: 'Adarsh Nagar,Delhi',city: 'Adarsh Nagar',region: 'Adarsh Nagar',postal_code: '3000',country: "India",phone: '01822531439',password: 'Ritik',confirmPassword: 'Ritik'})
  }

  const login = async () => {
    return await request(server)
                 .post('/api/customar/login')
                 .send({email,password: 'Ritik'})
  }

  const updateExec = async (token,password="Ritik") => {
    return await request(server)
      .put('/api/customar')
      .set('virtual_customar_token',token)
      .send({name,email,address: 'Adarsh Nagar,Delhi',city: 'Adarsh Nagar',region: 'Adarsh Nagar',postal_code: '3000',country: "India",phone: '01822531439',password})
  }
  ...
  describe('Put /api/customar',() => {
    ...
    it('should return 400 if password is invalid ',async () => {
      await exec()
      const loged = await login()
      const res = await updateExec(loged.text,'Ritikjr')
      expect(res.status).toBe(400)
    })
