...
describe('Admin Routes',() => {
  ...
  const exec = async () => {
    return await request(server)
      ...
      .send({userName,name: 'Ritik Singh',password: 'Ritik@123',confirmPassword: 'Ritik@123'})
  }

  const loginExec = async () => {
    return await request(server)
      .post('/api/admin/login')
      .send({userName,password: 'Ritik@123'})
  }
  ...
  describe('Post /api/admin/',() => {
    ...
    describe('/register',() => {
      ...
      it('should save data',async () => {
        await exec();
        const data = await Admin.findOne({userName: 'Ritik'})
        expect(data).toHaveProperty('_id')
        expect(data).toHaveProperty('userName','Ritik')
      })
  ...
  describe('Get /api/admin',() => {
    ...
    it('Get/ me: should return loged admin data',async () => {
      ...
      expect(res.body).toHaveProperty('userName','Ritik')
    })
  ...
  describe('Put /api/admin',() => {
    ...
    const updateExec = async () => {
      ...
      return  await request(server)
        .put('/api/admin')
        .set('admin_token',login.text)
        .send({userName,name,oldPassword: 'Ritik@123',newPassword: 'Ritik@1234'})
    }
    ...
    it('should return 200 if everything is ok as expected',async () => {
      ...
      expect(res.status).toBe(200)
      expect(res.body).toHaveProperty('userName','Ritik')
      expect(res.body).toHaveProperty('name','Ritik Singh')
    })
    ...
    it('should return 401 error if token is not provided',async () => {
      ...
      const res = await request(server)
        .put('/api/admin')
        .send({userName,name,oldPassword: 'Ritik@1234',newPassword: 'Ritik@123'})
      expect(res.status).toBe(401)
    })
    ...
    it('should return 400 error if token is not valid',async () => {
      ...
      const res = await request(server)
        ...
        .send({userName,name,oldPassword: 'Ritik@123',newPassword: 'Ritik@123'})
      expect(res.status).toBe(400)
    })
  })
